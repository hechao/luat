...
--测试时请搭建自己的服务器
local SCK_IDX,PROT,ADDR,PORT = 1,"TCP","121.40.170.41",57315
...
local function sndcb(item,result)
    ...
    elseif item.para=="RCDRPT" then
		sys.dispatch("RCD_SEND_CNF")
	end
end
...
function rcddatarpt(data)
    print("rcddatarpt",linksta,data,slen(data))
	if linksta then
		snd(data,"RCDRPT")		
	end
end
