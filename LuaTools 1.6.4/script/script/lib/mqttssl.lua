...
function mqttdiscb(sckidx,result,tpara)
	...
	socketssl.disconnect(sckidx,tpara.usertag)
end
...
功能  ：socket状态的处理函数
参数  ：
        idx：number类型，socketssl中维护的socket idx，跟调用socketssl.connect时传入的第一个参数相同，程序可以忽略不处理
        evt：string类型，消息事件类型
...
功能  ：socket接收数据的处理函数
参数  ：
        idx ：socketssl中维护的socket idx，跟调用socketssl.connect时传入的第一个参数相同，程序可以忽略不处理
        data：接收到的数据
返回值：无
]]
...
function create(prot,host,port,chksvrcrt,ver)
	...
	local mqtt_client =
	{
		...
		port=port,	
		chksvrcrt = chksvrcrt,
		sckidx=socketssl.SCK_MAX_CNT-#tclients,
		sckconning=false,
		sckconnected=false,
		sckreconncnt=0,
		sckreconncyclecnt=0,
